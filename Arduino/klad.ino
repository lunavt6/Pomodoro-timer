...
void loop() {
  ...
  if (timerLoopt) {
    if (millis() - laatstetijd >= 1000) { // nadat 1 seconde verlopen is mag de seconde-waarde aangepast worden
      ...
      if (j > 0 && j < 6 or j > 30 && j < 36){ // j moet gedurende deze intervallen pauze aangeven
        if (j == 5 or j == 35){ // op de led wisseling zal er een beep geluid te horen zijn
      ...
      if (j > 5 && j < 30 or j > 35 && j <= 59) { // j moet gedurende deze intervallen studeren aangeven
  ...
  //}
  if (timerLoopt){ // als de timer loopt en het scherm staat aan, dan is de groene led aan
      if (pauze == true){
  ...
  else { // als het scherm aanstaat en de timer niet loopt
      ledState=LED_UIT_G_LED_UIT_R;
  }
